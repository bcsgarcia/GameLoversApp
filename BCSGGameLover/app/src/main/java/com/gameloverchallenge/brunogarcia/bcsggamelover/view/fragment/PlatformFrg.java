...
public class PlatformFrg extends Fragment implements RecyclerViewOnClickListenerHack {
    ...
    private void initializeComponents(){
        ...
        rvListGames = (RecyclerView) mRootView.findViewById(R.id.rvListGames);
        rvListGames.setLayoutManager(mGLM);

        rvListGames.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
        ...
        rvListGames.setHasFixedSize(true);
        ...
        rvListGames.setAdapter(mAdapter);
    ...
    private void getNextGames(){
        try {
            ...
            for (int i = CacheData.getInstance(getContext()).getListGame(mPlatform.getId()).size(); i <= idxConsulta;  i++  ){
                try {
            ...
            String strIds = listId.toString().replace("[","").replace("]","").replace(" ","");
            GamesAdapter adapter = (GamesAdapter) rvListGames.getAdapter();
            CacheData.getInstance(getContext()).populateGames(strIds, mPlatform, adapter, progress);
